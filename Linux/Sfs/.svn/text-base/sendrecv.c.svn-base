...
int aceptarConexion (int iSocket)
{
	...
	if (iSocket_aceptado == -1) 
	{
       ...
       printf("Servidor WEBSTORE: nueva conexion de %s en socket %d\n", inet_ntoa(pin.sin_addr), iSocket_aceptado);	                    
...
int realizarHandshake(int iSocket, FILE *archLog)
{

    HEADERIRCIPC header;

    generarID(header.cpDescriptorID);

    header.ucPayloadDescriptor = 0x92;
    header.uiPayloadLength =0;

    if(enviarH(iSocket, header) == -1)
        return -1;
	
	enviarH(iSocket,header);
	enviarAlgo(iSocket,(unsigned char*)"SFS",header.uiPayloadLength);
	
    if(recibirH(iSocket, &header) == -1)
        return -1;

	if(header.ucPayloadDescriptor == 0x98)
		return 1;
		
	header.ucPayloadDescriptor = 0x92;
    header.uiPayloadLength =strlen("SFS");
